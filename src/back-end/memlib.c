...
void init_mem_man(){
	#ifdef DEBUG
	dbprint(MEMLIBC,"Initializing Memory Manager",STR, (void*)"");
/*	list_mem(INT);
	list_mem(FLOAT);
	list_mem(STR);*/
	list_mem(OBJ, TRUE);
...
//	#endif
	if( (*tag = get_avail_mem_tag(intype)) != -1){	
		switch(intype){
			case INT:
						#ifdef DEBUG
						dbprint(MEMLIBC,"Requesting Memory for an Integer",STR,"");
						#endif
						temp = NULL;
						...
						#ifdef DEBUG
						dbprint(MEMLIBC,"Integer Address after creation",LONG,(void*)&mem_manager.int_address[*tag]);
						#endif
			...
			case STR:
						#ifdef DEBUG
						dbprint(MEMLIBC,"Requesting Memory for a String/Character",STR,"");
						#endif
						tempchar = NULL;
						...
						#ifdef DEBUG
						dbprint(MEMLIBC,"String Address after creation",LONG,(void*)&mem_manager.char_address[*tag]);
						#endif
						if(mem_manager.cs_num < MAX_MEM) mem_manager.cs_num += 1;
						return (void*)tempchar;
			case FLOAT: 
						#ifdef DEBUG
						dbprint(MEMLIBC,"Requesting Memory for a Float",STR,"");
						#endif
						tempfloat = NULL;
						...
						#ifdef DEBUG
						dbprint(MEMLIBC,"Float Address after creation",LONG,(void*)&mem_manager.float_address[*tag]);
						#endif
						if(mem_manager.f_num < MAX_MEM) mem_manager.f_num += 1; 
						return (void*)tempfloat;
			case GENERIC:
						#ifdef DEBUG
						dbprint(MEMLIBC,"Requesting Memory for a Generic Memory Type",GENERIC,"");
						#endif
						tempgeneric = NULL;
						...
						#ifdef DEBUG
						dbprint(MEMLIBC,"Generic Memory Object Address after creation",LONG,(void*)&mem_manager.generic_address[*tag]);
						#endif
		...
		#ifdef DEBUG
		dbprint(MEMLIBC,"Couldn't get a memory unit tag",INT, (void*)&tag);
		#endif
		return NULL;
	}
}
...
//	#endif
	...
	if( (otag = get_avail_mem_tag(OBJ)) != -1){	
		...
		switch(intype){
			case INT: 
						#ifdef DEBUG
						dbprint(MEMLIBC,"Requesting Memory for an Integer Object",STR,"");
						#endif
...
//							#endif
							...
							#ifdef DEBUG
							dbprint(MEMLIBC,"Amount of allocated Objects", INT, &mem_manager.o_amount);
							#endif
							mem_manager.otype[otag] = INT;
							if(mem_manager.o_num < MAX_MEM) mem_manager.o_num += 1;
							#ifdef DEBUG
							dbprint(MEMLIBC,"Object Tag: ", INT, (void*)&otag);
							//dbprint(MEMLIBC,"Object Starting Value:", INT, (void*) temp->value);
							dbprint(MEMLIBC,"Object Address:", LONG, (void*)&mem_manager.obj_address[otag]);
							dbprint(MEMLIBC,"Integer Tag: ", INT, (void*)&temp->tag);
							dbprint(MEMLIBC,"Integer Starting Value:", INT, (void*) temp->value);
							dbprint(MEMLIBC,"Integer Address:", LONG, (void*)&mem_manager.int_address[temp->tag]);
							dbprint(MEMLIBC,"",STR,"");
							#endif
							return (void*)temp;
						}
			...
			case FLOAT:
						#ifdef DEBUG
						dbprint(MEMLIBC,"Requesting Memory for a Float Object",STR,"");
						#endif
						...
						if( tempfloat->tag != -1){
							...
							//dbprint(MEMLIBC,"Object Starting Value:", INT, (void*) temp->value);
							dbprint(MEMLIBC,"Object Address:", LONG, (void*)&mem_manager.obj_address[otag]);
							dbprint(MEMLIBC,"Float Tag: ", INT, (void*)&(tempfloat->tag));
							dbprint(MEMLIBC,"FLoat Starting Value:", FLOAT, (void*) tempfloat->value);
							dbprint(MEMLIBC,"Float Address:", LONG, (void*)&mem_manager.float_address[tempfloat->tag]);
							dbprint(MEMLIBC,"",STR,"");
							#endif
							return (void*)tempfloat;
						}
			...
			case CHAR:
						#ifdef DEBUG
						dbprint(MEMLIBC,"Requesting Memory for a String/Character Object",STR,"");
						#endif
						...
						if( tempstr->tag != -1){
							...
							//dbprint(MEMLIBC,"Object Starting Value:", INT, (void*) temp->value);
							dbprint(MEMLIBC,"Object Address:", LONG, (void*)&mem_manager.obj_address[otag]);
							dbprint(MEMLIBC,"String/Character Tag: ", INT, (void*)&tempstr->tag);
							dbprint(MEMLIBC,"String/Character Starting Value:", STR, (void*) tempstr->value);
							dbprint(MEMLIBC,"String/Character Address:", LONG, (void*)&mem_manager.char_address[tempstr->tag]);
							dbprint(MEMLIBC,"",STR,"");
							#endif
							return (void*)tempstr;
						}
...
int releaseall(){
#ifdef DEBUG
	dbprint(MEMLIBC,"Releasing All Memory",STR," : listing all memory before release");
//	list_mem(INT);
//	list_mem(FLOAT);
//	list_mem(STR);
#endif	
...
/*	for(int i = 0; i < mem_manager.o_num;i++){
	...
	mem_manager.o_num = 0;*/
#ifdef DEBUG
	dbprint(MEMLIBC,"Release All Memory Checking for errors",STR,"");
//	list_mem(INT);
//	list_mem(FLOAT);
//	list_mem(STR);
#endif
...
int release(void * mem, typecg intype, int tag){
	...
	if(mem !=NULL){
		if(tag > MAX_MEM){
				#ifdef DEBUG
				dbprint(MEMLIBC,"tag greater than MAX_MEM", INT, &tag);
				#endif	
				 return -1;
			 }
			switch(intype){
				case INT: 	
							#ifdef DEBUG
							dbprint(MEMLIBC,"Releasing Integer Memory with tag", INT, &tag);
							//list_i_mem();
							#endif	
							if((tag > mem_manager.i_num  && tag < MAX_MEM) || tag < 0){
								#ifdef DEBUG
								dbprint(MEMLIBC,"tag greater than i_num or tag < 0", INT, &tag);
								#endif	
								 return -1;
							 }
							if(mem_manager.intbucket[tag] != NULL){
								if(mem_manager.intref[tag] == 1){ 
									...
									#ifdef DEBUG
									dbprint(MEMLIBC,"Reducing Reference Count from ",INT,&mem_manager.intref[tag]);
									#endif
									 mem_manager.intref[tag] -= 1;
								}
								 else{
							...
							#ifdef DEBUG
							dbprint(MEMLIBC,"Checking for errors on release",STR,"");
							//list_i_mem();
							#endif
							return 0;
				case CHAR:
				case STR:
							#ifdef DEBUG
							dbprint(MEMLIBC,"Releasing String/Character Memory with tag", INT, &tag);
							//list_cs_mem();
							#endif	
				...
				case FLOAT: 
							#ifdef DEBUG
							dbprint(MEMLIBC,"Releasing Float Memory with tag", INT, &tag);
							//list_f_mem();
							#endif	
				...
				case GENERIC:
							#ifdef DEBUG
							dbprint(MEMLIBC,"Releasing Generic Memory with tag", INT, &tag);
							//list_cs_mem();
							#endif	
				...
				default:
							#ifdef DEBUG
							dbprint(MEMLIBC,"WRONG TYPE",STR,"");
							#endif
							return -1;
			}
		...
		#ifdef DEBUG
		dbprint(MEMLIBC,"Already Released",STR,"");
		#endif
		return 0;
	}
}
	...
	if(obj != NULL){
		switch(intype){
			case INT:
						...
						otag = temp->o_tag;
#ifdef DEBUG
						dbprint(MEMLIBC,"found tag ", INT, &tag);
						dbprint(MEMLIBC,"found obj tag ", INT, &otag);
#endif
						if(mem_manager.objref[otag] > 0 && &mem_manager.objbucket[otag] != NULL){
							if(mem_manager.objref[otag] == 1){ 
								#ifdef DEBUG
								dbprint(MEMLIBC,"Releasing Integer Object ",STR,"");
								#endif
								...
								#ifdef DEBUG
								dbprint(MEMLIBC,"Reducing Reference Count from ",INT,&mem_manager.objref[otag]);
								#endif
								 mem_manager.objref[otag] -= 1;
							}
							 else{
								#ifdef DEBUG
								 dbprint(MEMLIBC,"ERROR NEGTIVE REFERENCES",STR,"");
								#endif
								 return -1;
								 ;
							}
			...
			case FLOAT:
						...
						if(mem_manager.objref[otag] > 0 && &mem_manager.objbucket[otag] != NULL){
							if(mem_manager.objref[otag] == 1){ 
								#ifdef DEBUG
								dbprint(MEMLIBC,"Releasing Float Object ",STR,"");
								#endif
								mem_manager.objref[otag] = 0;
								mem_manager.objhash[otag] = 0;
								mem_manager.obj_address[otag] = 0;
			...
			case CHAR:
						...
						if(mem_manager.objref[otag] > 0 && &mem_manager.objbucket[otag] != NULL){
							if(mem_manager.objref[otag] == 1){ 
								#ifdef DEBUG
								dbprint(MEMLIBC,"Releasing String/Character Object ",STR,"");
								#endif
								mem_manager.objref[otag] = 0;
								mem_manager.objhash[otag] = 0;
								mem_manager.obj_address[otag] = 0;
...
int releaseallobj(){
	int range;
	#ifdef DEBUG
	dbprint(MEMLIBC,"Releasing All Memory Objects : listing all memory before release", STR, "");
	dbprint(MEMLIBC,"How many Objects through the o_num variable", INT, (void*)&mem_manager.o_num);
	#endif	
	if(mem_manager.o_num < MAX_MEM) range = MAX_MEM;
	else range = mem_manager.o_num;
	for(int i = 0; i < range;i++){
		#ifdef DEBUG
		dbprint(MEMLIBC,"Checking object hash to see if empty or not",INT,(void*)&mem_manager.objhash[i]);
		#endif
		if(mem_manager.objhash[i] == 1){
			...
			#ifdef DEBUG
			dbprint(MEMLIBC,"object bucket fall through",STR,"");
			#endif
			;
		}
	}
			...
			switch(intype){
				case INT:
							...
							*temp->value = *iholder->value;
							...
							#ifdef DEBUG
							dbprint(MEMLIBC,"Tag Given: ", INT, (void*)&temp->tag);
							dbprint(MEMLIBC,"Value Given:", INT, (void*) temp->value);
							dbprint(MEMLIBC,"Address Equals:", LONG, (void*)&temp->address);
							#endif
							return (void*)temp;
				case FLOAT:	
							fholder = (float_m*)obj;
							...
							*tempfloat->value = *fholder->value;
							...
							#ifdef DEBUG
							dbprint(MEMLIBC,"Tag Given: ", INT, (void*)&tempfloat->tag);
							dbprint(MEMLIBC,"Value Given:", FLOAT, (void*) tempfloat->value);
							dbprint(MEMLIBC,"Address Equals:", LONG, (void*)&tempfloat->address);
							#endif
							return (void*)tempfloat;
				...
				case CHAR:
							...
							*tempstr->value = *sholder->value;
							...
							#ifdef DEBUG
							dbprint(MEMLIBC,"Tag Given: ", INT, (void*)&tempstr->tag);
							dbprint(MEMLIBC,"Value Given:", STR, (void*) tempstr->value);
							dbprint(MEMLIBC,"Address Equals:", LONG, (void*)&tempstr->address);
							#endif
							return (void*)tempstr;
...
int mem_cleanup(){
	#ifdef DEBUG
	dbprint(MEMLIBC,"Running Memory Manager Cleanup",STR, (void*)"");
	list_mem(STR,FALSE);
	list_mem(INT,FALSE);
	list_mem(FLOAT,FALSE);
...
//	releaseallobj();
	...
	#ifdef DEBUG
	dbprint(MEMLIBC,"Done.",STR,"");
	dbprint(MEMLIBC,"Checking for Errors", STR,"");
	list_mem(STR,FALSE);
	list_mem(INT,FALSE);
	list_mem(FLOAT,FALSE);
	list_mem(GENERIC,FALSE);
...
#ifdef DEBUG
void list_i_mem(){
	dbprint(MEMLIBC,"Integer Memory ", INT, (void*)&mem_manager.i_amount);
	for(int i=0;i<mem_manager.i_num;i++){
		if(mem_manager.inthash[i] == 1){
			dbprint(MEMLIBC,"Allocated in slot", INT, &i);
			dbprint(MEMLIBC,"Integer value", INT, (mem_manager.intbucket[i]));
			dbprint(MEMLIBC,"Number of References", INT, &(mem_manager.intref[i]));
			dbprint(MEMLIBC,"Address Given", LONG, (void*)&(mem_manager.int_address[i]));
		}
	}
}
void list_f_mem(){
	dbprint(MEMLIBC,"Float Memory ", INT, (void*) &mem_manager.f_amount);
	for(int i=0;i<mem_manager.f_num;i++){
		if(mem_manager.floathash[i] == 1){
			 dbprint(MEMLIBC,"Allocated in slot", INT, &i);
			dbprint(MEMLIBC,"Float value", FLOAT, (void*) mem_manager.floatbucket[i]);
 			dbprint(MEMLIBC,"Number of References", INT, &mem_manager.floatref[i]);
			dbprint(MEMLIBC,"Address Given", LONG, (void*)&(mem_manager.float_address[i]));
		 }
	}
}

void list_g_mem(){
	dbprint(MEMLIBC,"Generic Memory Object Memory", INT, (void*)&mem_manager.g_amount);
	for(int i=0;i<mem_manager.g_num;i++){
		if(mem_manager.generichash[i] == 1){ 
			dbprint(MEMLIBC,"Allocated in slot", INT, &i);
			dbprint(MEMLIBC,"Size in Bytes", INT, &mem_manager.generic_size[i]);
//			dbprint(MEMLIBC,"String/Character value", STR, mem_manager.charbucket[i]);
			dbprint(MEMLIBC,"Number of References", INT, &mem_manager.genericref[i]);
			dbprint(MEMLIBC,"Address Given", LONG, (void*)&(mem_manager.generic_address[i]));
		}
	}
}
...
void list_cs_mem(){
	dbprint(MEMLIBC,"String/Character Memory ", INT, (void*)&mem_manager.cs_amount);
	for(int i=0;i<mem_manager.cs_num;i++){
		if(mem_manager.charhash[i] == 1){ 
			dbprint(MEMLIBC,"Allocated in slot", INT, &i);
			dbprint(MEMLIBC,"String/Character value", STR, mem_manager.charbucket[i]);
			dbprint(MEMLIBC,"Number of References", INT, &mem_manager.charref[i]);
			dbprint(MEMLIBC,"Address Given", LONG, (void*)&(mem_manager.char_address[i]));
		}
	}
}
void list_o_mem(boolcg SUPPRESS_EMPTY_HASH){
	...
	tempfloat = NULL;
	tempstr = NULL;
	dbprint(MEMLIBC,"-- Number of Objects Allocated --", INT, (void*)&mem_manager.o_amount);
	for(int i=0;i<MAX_MEM;i++){
		temp = NULL;
		tempfloat=NULL;
		tempstr=NULL;
		if(mem_manager.objhash[i] == 1){ 
			dbprint(MEMLIBC,"-  Object Allocated in slot  -", INT, &i);
			switch(mem_manager.otype[i]){
				case INT:
							temp = (int_m*)mem_manager.objbucket[i];
							if(mem_manager.inthash[temp->tag] == 1){
								dbprint(MEMLIBC,"- Integer Allocated for Object in slot", INT, &temp->tag);
								dbprint(MEMLIBC,"-- Integer value", INT, (mem_manager.intbucket[temp->tag]));
								dbprint(MEMLIBC,"-- Number of References", INT , &(mem_manager.intref[temp->tag]));
								dbprint(MEMLIBC,"-- Integer Address Given", LONG, (void*)&(mem_manager.int_address[temp->tag]));
							}
							break;
				case FLOAT:
						...
						if(mem_manager.floathash[ftag] == 1){
							 dbprint(MEMLIBC,"- Float Allocated for Object in slot", INT, &(ftag));
						 	dbprint(MEMLIBC,"-- Float value", FLOAT, (void*)  &mem_manager.floatbucket[ftag]);
		 					 dbprint(MEMLIBC,"-- Number of References", INT, &mem_manager.floatref[ftag]);
							 dbprint(MEMLIBC,"-- Float Address Given", LONG, (void*)&(mem_manager.float_address[ftag]));
						 }
						 break;
				case CHAR:
				case STR:
						...
						if(mem_manager.charhash[tempstr->tag] == 1){ 
							dbprint(MEMLIBC,"- String/Character Allocated for Object in slot", INT, &tempstr->tag);
							dbprint(MEMLIBC,"-- String/Character value", STR, mem_manager.charbucket[tempstr->tag]);
							dbprint(MEMLIBC,"-- Number of References", INT, &mem_manager.charref[tempstr->tag]);
							dbprint(MEMLIBC,"-- String/Character Address Given", LONG, (void*)&(mem_manager.char_address[tempstr->tag]));
						}
						break;
				case OBJ:
				default: break;
...
//			debugprint(2,"String/Character value", mem_manager.objbucket[i]);
			dbprint(MEMLIBC,"-Object Number of References", INT, &mem_manager.objref[i]);
			dbprint(MEMLIBC,"-Object Address Given", LONG, (void*)&(mem_manager.obj_address[i]));
		}
		else{
			if(!SUPPRESS_EMPTY_HASH) dbprint(MEMLIBC,"Object Hash is empty ",INT,&i);
		}
	}
}
